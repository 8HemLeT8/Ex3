...
public class List2types {
	...
	public static ArrayList<Types> makeTypes(ArrayList<ArrayList<String>> table) {
		...
		while (it.hasNext()) {
			...
			if(p.get(0).equals("F")) {
				Fruit tut = new Fruit(p.get(2),p.get(3),p.get(4),p.get(5));
				types.add(tut);
			}
			else if(p.get(0).equals("P")) {
				Packman hamudi = new Packman(p.get(2),p.get(3),p.get(4),p.get(5),p.get(6));
				types.add(hamudi);
			}
			else {
				System.out.println("you are wrong blyat");
			}
